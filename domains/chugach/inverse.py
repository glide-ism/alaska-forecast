...
model.adjoint_solver.fas_options.set(
        coarsest_steps=200, pre_steps=10,
        post_steps=100, finest_steps=50,
        relative_tolerance=1e-7, absolute_tolerance=1e-8, # Note that adjoint var
...
for level in range(MAX_LEVEL,MIN_LEVEL-1,-1):
    ...
    def evaluate_loss(i,compute_gradient=True,write_vti=True):
        ...
        J_L2_beta = 10*((log_beta - -2.303)**2).mean()
        ...
        J_bed = 0.0*(torch.sqrt((bed_at_obs - bed_obs)**2 + 100.0).mean() - 10.0)
