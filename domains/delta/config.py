...
CONFIG = GlacierConfig(
    ...
    lambda_bed=2.0e-6,
    lambda_e=2e-5,
    lambda_snow=Schedule(final=1e-5,ramp=lambda i,level: 0.0 if (i<0 and level==2) else 1e-5),
    lambda_dhdt=Schedule(final=1e-5,ramp=lambda i,level: 0.0 if (i<0 and level==2) else 1e-5),
    loss_scale=1e-3,
    s_smb=0.5,
    s_H = 10.0,
    ssa_damping=1.0,
