...
GRIDDED_FILENAME = "preprocessing/model_inputs/GLIDE_inputs.nc"
FLIGHTLINE_FILENAME = "preprocessing/model_inputs/flightlines.gpkg"
ANOMALY_FILENAME = "preprocessing/model_inputs/temperature_anomaly.nc"
...
bed_model_coarse = MaternPrior(n_levels=N_LEVELS,ny=ny,nx=nx,dx=dx)
bed_model_coarse.mg.parameters.sigma.set(1000.0)
bed_model_coarse.mg.parameters.l.set(5000.0)
bed_model_coarse.mg.parameters.nu.set(1)

bed_model_fine = MaternPrior(n_levels=N_LEVELS,ny=ny,nx=nx,dx=dx)
bed_model_fine.mg.parameters.sigma.set(300.0)
bed_model_fine.mg.parameters.l.set(500.0)
bed_model_fine.mg.parameters.nu.set(1)
...
bed = torch.tensor(mg[0].geometry.bed.data,
        device='cuda',requires_grad=False)
z_bed_c = GGaPPWhiten.apply(bed_model_coarse,bed)
z_bed_c.requires_grad_()
z_bed_f = torch.zeros_like(z_bed_c)
z_bed_f.requires_grad_()
...
Vx = 2*torch.as_tensor(V(mg[0].x_cell,10000,1.0)).to(torch.float32)
Vy = 2*torch.as_tensor(V(mg[0].y_cell,10000,1.0)).to(torch.float32)
...
for level in range(MAX_LEVEL,MIN_LEVEL-1,-1):
    ...
    optimizer = torch.optim.Adam([{'params':log_beta,'lr':0.05},
                                     {'params':z_bed_c,'lr':0.0003},
                                     {'params':z_bed_f,'lr':0.003},
    ...
    def evaluate_loss(i,compute_gradient=True,write_vti=True):
        ...
        bed = bed_map(bed_model_coarse,z_bed_c) + bed_map(bed_model_fine,z_bed_f)
        ...
        J_tikh_bed = 1e-5*((z_bed_c**2).sum() + (z_bed_f**2).sum())#1e-8*((gy**2).sum() + (gx**2).sum())*mg[0].dx**2
        ...
        J = J_srf + J_vel + J_extent + J_tikh_bed + J_tikh_beta + J_L2_beta + J_smb + J_bed
        ...
        print(f"{i}, {J.item():.2f}, {J_srf.item():.2f}, {J_vel.item():.2f}, {J_extent.item():.2f}, {J_tikh_bed.item():.2f}, {J_tikh_beta.item():.2f}, {J_L2_beta:.2f}, {J_smb:.2f}, {J_bed:.2f}")
        if compute_gradient:
            J.backward()
            #bed.grad[~(rgi_mask*domain_mask)] = 0.0
        return J
