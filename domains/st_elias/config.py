...
CONFIG = GlacierConfig(
    ...
    observations=(
        ...
        SnowlineSpec(weight=Schedule(final=1e-5, ramp=lambda i, level: 0.0 if (i < 50 and level == 3) else 1e-5),
                     s_smb=0.5),
        DhdtSpec(weight=Schedule(final=2e-6, ramp=lambda i, level: 0.0 if (i < 50 and level == 3) else 2e-6)),
    ),
    loss_scale=1e-3,
    ssa_damping=1.0,
    sliding_m=1.0,
    ...
    lr_z_bed=0.0175,
    pbias_prior = PriorHyperparams(sigma=0.02,l=10000.0, nu=1),
    depth_blend=0.1,
    lr_z_log_mf = 0.05,
    lr_z_log_rf = 0.05,
