...
index = cp.random.randint(1e8)
OUTPUT_PATH = f'{BASE_DIR}/inverse_white_v2/rto/rto_{index}/'
...
model.adjoint_solver.fas_options.set(
        coarsest_steps=200, pre_steps=10,
        post_steps=50, finest_steps=50,
        relative_tolerance=1e-3, absolute_tolerance=1e-7, # Note that adjoint var
...
#GEOM_START_PATH = f'{INPUT_PATH}/level_0/inverse_soln.nc'
#if GEOM_START_PATH is not None:
#    sol = xr.load_dataset(GEOM_START_PATH)
#    mg.state.H.set(sol.H,start_level=0)
#    mg.state.H_prev.set(sol.H,start_level=0)
...
if WARM_START_PATH is not None:
    ...
    z_log_beta = d['log_beta']
    z_bed = d['bed']
    z_bed_mean = d['bed_mean']
    z_precipitation_bias = d['precipitation_bias']
    z_log_mf = d['log_mf']
    z_log_rf = d['log_rf']
...
u_obs += torch.randn_like(u_obs)*sigma_u
v_obs += torch.randn_like(v_obs)*sigma_u
S_obs += torch.randn_like(S_obs)*sigma_s
bed_obs += torch.randn_like(bed_obs)*sigma_s
...
for level in range(MAX_LEVEL,MIN_LEVEL-1,-1):
    ...
    mask = (torch.rand(*rgi_mask.shape,device='cuda') < gaussian_blur(rgi_mask.to(torch.float).unsqueeze(0),27).squeeze()).to(torch.float32)
    ...
    z_bed_rto_mean = torch.randn_like(z_bed)
    z_bed_mean_rto_mean = torch.randn_like(z_bed_mean)
    z_log_beta_rto_mean = torch.randn_like(z_log_beta)
    z_pbias_rto_mean = torch.randn_like(z_precipitation_bias)

    z_log_mf_rto_mean = torch.randn_like(z_log_mf)
    z_log_rf_rto_mean = torch.randn_like(z_log_rf)

    #z_bed.data[:,:] += z_bed_rto_mean
    z_bed_mean.data[:,:] += z_bed_mean_rto_mean
    z_log_beta.data[:,:] += z_log_beta_rto_mean
    z_precipitation_bias.data[:,:] += z_pbias_rto_mean
    ...
    def evaluate_loss(i,compute_gradient=True,write_vti=True):
        ...
        J_prior_bed = loss_scale*((z_bed_ - z_bed_rto_mean)**2).sum()
        J_prior_bed_mean = loss_scale*((z_bed_mean - z_bed_mean_rto_mean)**2).sum()
        J_prior_beta = loss_scale*((z_log_beta - z_log_beta_rto_mean)**2).sum()
        J_prior_pbias = loss_scale*((z_precipitation_bias - z_pbias_rto_mean)**2).sum()
        J_prior_smb = (((log_rf - mu_log_rf)/sigma_log_rf) - z_log_rf_rto_mean)**2 + (((log_mf - mu_log_mf)/sigma_log_mf) - z_log_mf_rto_mean)**2
    ...
    for i in range(0,max_iters[level]):
        evaluate_loss(i,write_vti=True)  ## !!
        optimizer_adam.step()
        optimizer_sgd.step()
    #evaluate_loss(0,write_vti=True,compute_gradient=False) ## !!
